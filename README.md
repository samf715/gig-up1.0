# AngularJS Event Catalog Tutorial Application

## Overview

This application takes the developer through the process of building a web-application using
angular. The application is loosely based on the **Google Event Gallery**, which no longer
LOOOOOOOL
exists. Here is a historical reference: [Google Event Gallery on WayBack](http://web.archive.org/web/20131215082038/http://www.android.com/devices/).
...
- Add static html list with two events into index.html. We will convert this static page into
...
- Convert the static html list into dynamic one by:
  - creating `EventListCtrl` controller for the application.
  ...
  - converting the static HTML document into an Angular template with the use of the `ngRepeat`
    directive which iterates over the dataset of events.
...
- Add a search box to demonstrate how:
  ...
  - `ngRepeat` automatically shrinks and grows the number of events in the view.
...
- Add `age` property to each event in the data model.
- Add a `<select>` input to change the event list order.
...
- Replace the in-memory dataset with data loaded from the server (in
  the form of static `events.json` file).
  - The `events.json` file is loaded using the `$http` service.
...
- Add event images and links to new pages that show the event details.
...
- Create event list route:
  - Map `/events` route to `EventListCtrl` and `partials/events-list.html`.
  - Preserve existing `EventListCtrl` controller.
  - Move existing html from `index.html` to `partials/event-list.html`.
- Create event details route:
  - Map `/events/<event-id>` route to `EventDetailCtrl` and `partials/events-detail.html`.
...
- Implement `EventDetailCtrl` controller to fetch the details for a specific event from a JSON file
  using `$http` service.
- Update the template for the event detailed view.
- Add CSS to make the event details page look "pretty".


### step-9

- Add custom `checkmark` filter.
- Update event detail template to use `checkmark` filter.
- Add unit test for the filter.

### step-10

In the event detail view, clicking on a thumbnail image, changes the main event image to be the
large version of the thumbnail image.

- Define `mainImageUrl` model variable in the `EventDetailCtrl` and set its default value.
...
- Created a custom `Event` service that represents the `$resource` client.


### step-12

- Add animations to the application:
  - Animate changes to the event list, adding, removing and reordering events.
  - Animate changes to the main event image in the detail view.
