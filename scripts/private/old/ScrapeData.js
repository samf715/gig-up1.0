...
// Script which scrapes http://google.com/event and generates JSON files used by this application
...
var agent = httpAgent.create('www.google.com', ['/event/']);
var baseDir = __dirname + '/../app/events/';
var events = [];
...
//  console.log(htmlPage);                            
  ...
  jsdom.jQueryify(window, 'http://code.jquery.com/jquery-1.4.2.min.js', function (window, jquery) {
    ...
    if (events.length) {
      ...
      phone.gigs = c1.find('table:nth-child(1) th:contains("Availability")+td').text().trim().split(/\s*\n\s*/),
      phone.date = {
      ...
      body.find('ul.phonelist li.list').each(function(a){
        ...
        phone.imageUrl = 'http://google.com' + 
          jquery(this).find('img.event').attr('src');
      ...
      fs.writeSync(fs.openSync(baseDir + '.json', 'w'), JSON.stringify(events));
    }
    console.log(phone);
    agent.next();
  });
